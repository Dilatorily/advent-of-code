...
export const solution = (lines: string[], raceDuration = RACE_DURATION) => {
  ...
  const race: Record<
    string,
    { distance: number; remainingFlyTime: number; remainingRestTime: number }
  > = reindeers.reduce(
    (accumulator, { fly, name }) =>
      Object.assign(accumulator, {
        [name]: { distance: 0, remainingFlyTime: fly, remainingRestTime: 0 },
      }),
    {},
  );
  ...
  return Math.max(...Object.values(race).map(({ distance }) => distance));
};
