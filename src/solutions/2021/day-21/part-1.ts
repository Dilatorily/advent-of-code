const rollToSpaces = (roll: number, dice = 100) => ((roll - 1) % dice) + 1;
const turnToSpaces = (turn: number) =>
...
const playGame = ([initialPosition1, initialPosition2]: number[], win = 1000) => {
  const scores = [0, 0];
...
export const solution = (lines: string[]) => {
  const positions = lines.map((position) => parseInt(position.split(': ')[1], 10));

  return playGame(positions);
};
