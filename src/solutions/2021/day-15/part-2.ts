export const solution = (lines: string[]) => {
  const grid = lines.map((row) => row.split('').map((risk) => parseInt(risk, 10)));
  ...
  for (let i = 0; i < dimensions[1]; i += 1) {
    ...
    for (let j = 0; j < dimensions[0]; j += 1) {
      ...
      const expandedValue =
        ...
        Math.floor(j / originalDimensions[0]) +
        Math.floor(i / originalDimensions[1]);
  ...
  return distances[dimensions[1] - 1][dimensions[0] - 1];
};
