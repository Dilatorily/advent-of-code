...
const findVerticalReflection = (terrain: string[], smudges = 0) => {
...
const findHorizontalReflection = (terrain: string[], smudges = 0) => {
...
export const solution = (lines: string[]) => {
  ...
  const summarize = terrains
    .map((terrain) => {
      const verticalIndex = findVerticalReflection(terrain);

      if (verticalIndex >= 0) {
        return verticalIndex;
      }

      const horizontalIndex = findHorizontalReflection(terrain);
